...
function Cryptid.poll_equilibrium_key(seed_override)
	...
	if res == "c_base" then
		...
		local i = 1
		while res == "UNAVAILABLE" do
			res = pseudorandom_element(pool, (seed_override or "cry_equilibrium2") .. "_resample" .. i)
			i = i + 1
		end
	end
	return res, pcard
...
if SMODS and SMODS.add_booster_to_shop then
	...
	function SMODS.add_booster_to_shop(key, ...)
		...
		if key then
			assert(G.P_CENTERS[key], "Invalid booster key: " .. key)
		else
...
function create_card_for_shop(area)
	...
	if G.GAME.modifiers.cry_equilibrium and area == G.shop_jokers then
		...
		local key, pcard = Cryptid.poll_equilibrium_key("cry_equilibrium")
		local center = G.P_CENTERS[key]
...
function SMODS.upgrade_poker_hands(args)
	if (not args.func or G._CRY_DEFAULT_LEVELUP) and next(SMODS.find_card("j_cry_universum")) then --basically just whenever level_up_hand was called directly or upgrade_poker_hands would do so
		...
		args.func = function(base, hand, param, level_up)
			...
			return to_big(math.max(to_big(base) * (universum_mod ^ lv_amt), G.GAME.hands[hand]["s_" .. param] or 0)) --i have no fucking clue how to make this not infinity
		end
		if lvl_amt < 0 then
